/*此代码主要功能为：
 *通过RCON与服务器发包通信
 *本文件由SkipTime.exe文件调用
 * 
 *此文件由[ASY]Zyrain编写适配Linux
 *Squad红警_Stellarwolf改编适配Windows
*/
...
int send_rcon(int sock, int id, int command, char *string1, char *string2)
{
    ...
    ret = send(sock,(const char*)&size,sizeof(int),0);
    if(ret == -1) 
    {
        perror("send() failed:");
        return -1;
    }
    ret = send(sock,(const char*)&id,sizeof(int),0);
    if(ret == -1) 
    {
        perror("send() failed:");
        return -1;
    }
    ret = send(sock,(const char*)&command,sizeof(int),0);
    if(ret == -1) 
    {
        perror("send() failed:");
        return -1;
...
int recv_rcon(int sock, int timeout, int *id, int *command, char *string1,char *string2)
{
    ...
    *command=0xDEADBEEF;
    ...
    ret = recv(sock, (char*)&size, sizeof(int), 0);
    if(ret == -1) 
    {
        perror("recv() failed:");
        return -1;
    ...
    ret = recv(sock, (char*)id, sizeof(int),0);
    if(ret == -1) 
    {
        perror("recv() failed:");
        return -1;
    ...
    size-=ret;
    ret = recv(sock, (char*)command, sizeof(int),0);
    if(ret == -1) 
    {
        perror("recv() failed:");
        return -1;
...
int process_response(int sock) 
{
    ...
    switch(command) 
    {
        ...
        case SERVERDATA_RESPONSE_VALUE:
            printf("%s",string1);break;
        default:
            printf("Unexpected command: %d",command);break;
    };
}
...
int main(int argc, char **argv)
{
    ...
    for(arg = 1;arg<argc;arg++) 
    {
        ...
        {
            case 'a':
                strncpy(address, argv[arg]+2, 512);break;
            case 'p':
                port = atoi(argv[arg]+2);break;
            case 'P':
                strncpy(password, argv[arg]+2, 512);break;
            default:
